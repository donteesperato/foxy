...
//
var apps = express();
var https = require("http").Server(apps);
...
const DEFAULT_PIC = "https://i.imgur.com/fNnjnYE.png";
const options = {
...
var jsonParser = bodyParsers.json();
...
apps.get("/profile", (req, res) => {
  res.sendFile(__dirname + "/views/profile.html");
});

/* ENTERING */

apps.post("/register", jsonParser, function(req, res) {
  register(req, res, data, bcrypt, DEFAULT_PIC, utils, options);
});

apps.post("/login", jsonParser, function(req, res) {
  login(req, res, data, bcrypt, utils, options, usersOnline);
});

apps.post("/connect", jsonParser, function(req, res) {
  login(req, res, data, bcrypt, utils, options);
});

apps.post("/profile", jsonParser, function(req, res) {
  profile(req, res, data, bcrypt);
});
...
app.post("/paylasim", (req, res) => {
  ...
  client.channels.cache.get(IDler.kodLogKanalı).send(
    ...
    .addField("Kod Sayfası", `[Tıkla!](https://psychopath-techonology.ml/${obj.kod_rank}/${obj.id})`));
